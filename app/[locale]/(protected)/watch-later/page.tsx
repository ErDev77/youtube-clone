...
export default function WatchLaterPage() {
	...
	async function handleRemove(videoId: string) {
		...
		try {
			const res = await fetch('/api/me/watch-later', {
				method: 'DELETE',
	...
	function playAll() {
		...
		router.push(`/en/watch/${processed[0].id}?queue=watchlater&index=0`)
	}

	function playAt(index: number) {
		router.push(
			`/en/watch/${processed[index].id}?queue=watchlater&index=${index}`,
		)
	}
		...
		<UserLayout>
			...
			{loading ? (
				...
				>
					...
					>
						Save videos to watch later using the{' '}
						<strong style={{ color: '#888' }}>Watch Later</strong> button on any
						video page.
					</p>
					<Link
						href='/en'
